...
test('message send keeps attachment metadata and delivery states',async()=>{const a=new DemoClient();const c=await a.request('POST','/chats',{mode:'general'});const next=await a.request('POST',`/chats/${c.id}/messages`,{content:'Revisa esto',attachments:[{id:'att-1',name:'ejemplo.pdf',size:1200,type:'application/pdf'}]});assert.equal(next.messages[0].role,'user');assert.equal(next.messages[0].status,'delivered');assert.equal(next.messages[0].attachments[0].name,'ejemplo.pdf');assert.equal(next.messages[1].status,'complete');});
test('file-only send is allowed and oversized attachment is rejected',async()=>{const a=new DemoClient();const c=await a.request('POST','/chats',{mode:'sandbox'});const next=await a.request('POST',`/chats/${c.id}/messages`,{content:'',attachments:[{id:'att-1',name:'nota.txt',size:24,type:'text/plain'}]});assert.equal(next.messages[0].content,'');assert.equal(next.title,'nota.txt');await assert.rejects(()=>a.request('POST',`/chats/${c.id}/messages`,{content:'x',attachments:[{id:'huge',name:'grande.bin',size:26*1024*1024,type:'application/octet-stream'}]}),/25 MiB/);});
test('project chats auto-save, pin and archive without changing permissions',async()=>{const a=new DemoClient();const projects=await a.request('GET','/projects');const project=projects.find(p=>p.agentId==='universidad');assert.ok(project);const c=await a.request('POST','/chats',{mode:'agent',agentId:'universidad',projectId:project.id});assert.equal(c.saved,true);assert.equal(c.projectId,project.id);assert.equal(c.pinned,false);const pinned=await a.request('PATCH',`/chats/${c.id}`,{pinned:true});assert.equal(pinned.pinned,true);assert.equal(pinned.mode,'agent');assert.equal(pinned.agentId,'universidad');assert.equal((await a.request('GET',`/chats?projectId=${project.id}&pinned=true`)).length,1);await a.request('PATCH',`/chats/${c.id}`,{archived:true});assert.equal((await a.request('GET',`/chats?projectId=${project.id}`)).length,0);assert.equal((await a.request('GET',`/chats?projectId=${project.id}&archived=true`)).length,1);});
test('invalid project cannot be assigned to a conversation',async()=>{const a=new DemoClient();await assert.rejects(()=>a.request('POST','/chats',{mode:'agent',agentId:'vivienda',projectId:'project-root'}),/Proyecto/);});
test('saving is separate from knowledge extraction',async()=>{const a=new DemoClient();const before=(await a.request('GET','/knowledge')).length;const c=await a.request('POST','/chats',{mode:'general'});await a.request('POST',`/chats/${c.id}/save`,{expectedRevision:1,category:'general'});assert.equal((await a.request('GET','/knowledge')).length,before);});
...
test('OpenAPI contract has distinct operations, authentication and resolvable schema references',async()=>{
 ...
 assert.equal(routes.length,31);assert.equal(new Set(routes.map(r=>r[2])).size,31);
